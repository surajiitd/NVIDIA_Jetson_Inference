...
if args.dataset == 'kitti' or args.dataset == 'nyu' or \
    args.dataset == '12scenes' or args.dataset == 'iitd' or \
    args.dataset == 'eth3d' :
...
def get_scale_shift(prediction, target,  min_depth, max_depth):
    ...
    #print("median target ",np.median(target[mask]))
    #print("median prediction ",np.median(prediction[mask]))
...
def test(params):
    ...
    with open(args.filenames_file_eval) as f:
        lines = f.readlines()
    sample_img = cv2.imread(os.path.join(args.data_path_eval,lines[0].split()[0]),-1)
    args.input_height, args.input_width,_ = sample_img.shape
    print(args.input_height, args.input_width)
    ...
    save_name = 'visualisations/result_' + args.dataset + "_" + lines[0].split(" ")[0].split("/")[0]

    os.makedirs(save_name,exist_ok=True)
    
    
    max_thresholds_for_depth = [10,8,7,6,5,4,3.5,3,2.5,2,1.5,1]
    try:
        for max_depth_threshold in max_thresholds_for_depth:
            os.makedirs(save_name + f'/raw{max_depth_threshold}',exist_ok=True)
    ...
    with torch.no_grad():
        for i, sample in enumerate(tqdm(dataloader.data)):
            ...
            # print(type(gt_depth.numpy().squeeze()))
            # print(gt_depth.numpy().squeeze().shape)
            #print_minmax(gt_depth.numpy().squeeze(),"gt_depth")
            #sys.exit(0)
            ...
            gt_depth = (gt_depth.numpy().squeeze() * 5000.0).astype(np.uint16)
            for max_depth_threshold in max_thresholds_for_depth:
                print("max_depth_threshold =",max_depth_threshold)
                args.max_depth_eval = max_depth_threshold
                ...
                pred_depth[pred_depth > args.max_depth_eval] = 0
                ...
                #print_minmax(pred_depth,"pred_depth")
                scale,shift = get_scale_shift(pred_depth/1000.0, gt_depth/5000.0, min_depth=args.min_depth_eval, max_depth=args.max_depth_eval )
                pred_depth = (pred_depth * scale + shift).astype(np.uint16)
                print("scale = ",scale)

                #print_minmax(pred_depth,"pred_depth")
                #sys.exit(0)
                pred_depth = cv2.resize(pred_depth, (args.input_width, args.input_height))
                
                    

                #save pred_depth, image and gt_depths
                #print_minmax(gt_depth,"gt_depth")
                image_name = os.path.basename(lines[i].split(" ")[1].strip())
                #print(image_name)
                #print("lines[i].split(" ")[1] =",lines[i].split(" ")[1].strip())
                #sys.exit(0)
                cv2.imwrite(save_name + f'/raw{args.max_depth_eval}/{image_name}',pred_depth)
