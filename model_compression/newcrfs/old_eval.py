...
# def get_mono_ratio(pred_depth, gt):
...
#     """
#     mask = gt>0
#     return np.median(gt[mask]) / np.median(cv2.resize(pred_depth, (gt.shape[1], gt.shape[0]))[mask])
def get_mono_ratio(prediction, target):
    """Returns the median scaling factor from gt_depth and pred_depth,
        Tells by what scale factor you should scale up(multipy) your pred_depth.
    """
    mask = np.logical_and(target>1e-3 , target<6)
...
def eval(model, dataloader_eval, post_process=False):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        ...
        # scale_factor = get_mono_ratio(pred_depth, gt_depth)
        # pred_depth = pred_depth * scale_factor
        pred_depth = get_mono_ratio(pred_depth, gt_depth)
        ...
        top_abs_rel_indices.append([measures[1],valid_index])
        top_rmse_indices.append([measures[3], valid_index])
        valid_index += 1

        eval_measures[:9] += torch.tensor(measures).cuda()
        eval_measures[9] += 1
    top_abs_rel_indices.sort(reverse=True)
    top_rmse_indices.sort(reverse=True)

    top_error_store_path = "visualisations/top_error_indices"
    seq_type = "none"
    os.makedirs(top_error_store_path,exist_ok=True)
    if args.dataset == "kitti":
        seq_type = args.dataset + ("gray" if args.gray else "")
    elif args.dataset == "iitd":
        seq_type = args.dataset + "_" + args.filenames_file_eval.split("/")[-1].split("_")[-1].split(".")[-2] #indoor or outdoor
    
    np.save(top_error_store_path + "/top_abs_rel_"+seq_type, np.array(top_abs_rel_indices))
    np.save(top_error_store_path + "/top_rmse_"+seq_type, np.array(top_rmse_indices))
