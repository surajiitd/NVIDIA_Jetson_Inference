...
- installing pytorch 
    ...
    - Used these packages from global python: tensorrt, pycuda and cv2.
    - `import cv2` was throwing this error: `ImportError: /lib/aarch64-linux-gnu/libp11-kit.so.0: undefined symbol: ffi_type_pointer, version LIBFFI_BASE_7.0`. then I found this soln from this [comment of stackoverflow](https://stackoverflow.com/questions/75045632/apt-update-failed-libp11-kit-so-0-undefined-symbol-ffi-type-pointer-version#comment136434458_75045665) `rm ${CONDA_PREFIX}/lib/libffi.7.so ${CONDA_PREFIX}/lib/libffi.so.7` (run this inside your desired conda env). then I was be able to `import cv2` from my conda env. (I think after this, the cv2 I am referring to is the global one.... as I have done same import cv2 from a brand new conda environment which doesn't have opencv-python).
